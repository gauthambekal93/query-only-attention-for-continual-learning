...
class GnT(object):
    ...
    def __init__(
        ...
        """
        self.replacement_rate = replacement_rate  #1e-6
        ...
        """
        self.util = [torch.zeros(self.net[i * 2].out_features).to(self.device) for i in range(self.num_hidden_layers)]  #initialize utility to zero for all the hidden layer neurons
        self.bias_corrected_util = \
            [torch.zeros(self.net[i * 2].out_features).to(self.device) for i in range(self.num_hidden_layers)] #initialize utility to zero for all the hidden layer neurons
        self.ages = [torch.zeros(self.net[i * 2].out_features).to(self.device) for i in range(self.num_hidden_layers)] # #initialize ages to zero for all the hidden layer neurons
        self.m = torch.nn.Softmax(dim=1)
        self.mean_feature_act = [torch.zeros(self.net[i * 2].out_features).to(self.device) for i in range(self.num_hidden_layers)] # [tensor([0., 0., 0., 0., 0.])]
        self.accumulated_num_features_to_replace = [0 for i in range(self.num_hidden_layers)]  # [0] #we get total accumulation which adds up over training steps/ time
    ...
    def compute_bounds(self, hidden_activation, init='kaiming'):   #need to understand conceptually what does kaiming initialization do
        ...
        bounds.append(1 * sqrt(3 / self.net[self.num_hidden_layers * 2].in_features))   #[0.5477225575051662, 0.7745966692414834]
        return bounds

    def update_utility(self, layer_idx=0, features=None, next_features=None):  #layer index is the hidden layer number, feature is "h" variable in paper
        with torch.no_grad():
            self.util[layer_idx] = self.util[layer_idx]* self.decay_rate  # #this should be part of equation 7
            """
            Adam-style bias correction. Equation 5 (contribution utility) is explictly not needed since equation 6 and 7 captures it.
            """
            bias_correction = 1 - self.decay_rate ** self.ages[layer_idx]  #this is denominator in equation 4

            self.mean_feature_act[layer_idx] *= self.decay_rate                                 #this is part of equation 3
            self.mean_feature_act[layer_idx] -= - (1 - self.decay_rate) * features.mean(dim=0)  #this is part of equation 3
            bias_corrected_act = self.mean_feature_act[layer_idx] / bias_correction  #this is equation 4

            current_layer = self.net[layer_idx * 2]  #this will store the current layer
            next_layer = self.net[layer_idx * 2 + 2]  #this will store the next layer
            output_wight_mag = next_layer.weight.data.abs().mean(dim=0)    #this should be the numerator part for equation 6
            input_wight_mag = current_layer.weight.data.abs().mean(dim=1)  #this should be the denominator part for equation 6
            ...
            if self.util_type == 'weight':
                ...
                new_util = output_wight_mag * (features - bias_corrected_act).abs().mean(dim=0) / input_wight_mag  #this should be equation 6
            ...
            self.util[layer_idx] = self.util[layer_idx]  + (1 - self.decay_rate) * new_util   #this should be part of equation 7

            """
            Adam-style bias correction
            """
            self.bias_corrected_util[layer_idx] = self.util[layer_idx] / bias_correction   #this should be equation 8
    ...
    def test_features(self, features):
        ...
        for i in range(self.num_hidden_layers):
            self.ages[i] += 1    #we increment the age of hidden layer neurons 
            """
            ...
            if eligible_feature_indices.shape[0] == 0:   #if no neurons have reached machurity we donot update neurons in current layer
                continue
            num_new_features_to_replace = self.replacement_rate*eligible_feature_indices.shape[0] #we are multiplying replacement rate with total no. of neurons in the hidden layer
            self.accumulated_num_features_to_replace[i] += num_new_features_to_replace #we get total accumulation which adds up over training steps/ time

            """
            Case when the number of features to be replaced is between 0 and 1.
            """
            if self.accumulate:   #by default it is False
            ...
            features_to_replace[i] = new_features_to_replace  #neurons to be replaced in a given layer
            num_features_to_replace[i] = num_new_features_to_replace   #number of neurons to be replaced in a given layer.
    ...
    def gen_new_features(self, features_to_replace, num_features_to_replace):
        ...
        with torch.no_grad():
            for i in range(self.num_hidden_layers):
                ...
                # noinspection PyArgumentList  Below we have updated the neuron with noisy weights 
                ...
                self.ages[i][features_to_replace[i]] = 0  #we reset the age to zero for the neurons which were reset to random values
    ...
    def gen_and_test(self, features):  #features is the output of the hidden layers
        """
