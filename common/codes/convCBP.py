...
class ConvCBP(object):
    ...
    def learn(self, x, target):
        ...
        accuracy = 100*(output.argmax(dim =1 )== target.argmax(dim = 1)).float().mean()
        
        return loss.detach(), accuracy
    
    def test(self, data_dict_val ):
         
        x, target = data_dict_val["test_x"], data_dict_val["test_y"]
        ...
        accuracies =  100*(output.argmax(dim =1 )== target.argmax(dim = 1)).float().mean()
