...
class DataManager:
     ...
     def __init__(self, device, root, data_dir, flip_after, num_data_points, num_old_task_window, num_datapoints_per_timestep, train_size, buffer_size, samples_from_buffer): 
         ...
         self.buffer_z = torch.empty(buffer_size).to(self.device)
     ...
     def fill_buffer(self, x, z, y):
         ...
         for i in range(B):
             ...
             if self.buffer_counter < self.buffer_size:
                 ...
                 self.buffer_z [self.buffer_counter] = z[i].clone()
     ...
     def get_data(self):
         ...
         return self.buffer_x[sample_ids], self.buffer_z[sample_ids].reshape(-1, 1), self.buffer_y[sample_ids].reshape(-1, 1)
