...
class ERNetwork(nn.Module):
    ...
    def get_attention(self, query_x, query_y, support_x, support_y):
        ...
        for label in range(10):
            ...
            #we donot want to consider the relationship between query and itself when calculating the distance metric. This is the last entry in matrix
            normalized_attention[-1, -1] = 0 
            #label_specific_attention[label] =normalized_attention normalized_attention[-1][: -1]
            label_specific_attention[label] =normalized_attention 
