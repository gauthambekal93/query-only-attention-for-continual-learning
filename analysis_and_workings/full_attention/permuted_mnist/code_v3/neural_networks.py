...
class ERNetwork(nn.Module):
    ...
    def classify_images(self, query_x, query_y, support_x, support_y):
        ...
        k1 =  key_output_1.transpose(-2, -1)[:, :, :-1]
        
        m = torch.matmul(q1, k1).squeeze(1)
        
        base = query_y.unsqueeze(1) * 10          # (400, 1)
        
        offset = torch.arange(10, device=query_y.device)  # (10,)

        col_ids = base + offset                   # (400, 10)
        
        row_ids = torch.arange(0, len(m), 1).reshape(-1, 1)
        
        mask = torch.zeros(m.shape, dtype=torch.bool).to(m.device)
        
        mask[row_ids, col_ids] = True
    
        matched_theta_prime = m[mask].mean()
        
        unmatched_theta_prime = m[~mask].mean()

        delta = torch.abs(matched_theta_prime- unmatched_theta_prime).item()
    ...
    def prediction(self, data_manager_obj, query_x, query_y ):
        ...
        predictions, delta = self.classify_images(query_x, query_y, support_x, support_y )
