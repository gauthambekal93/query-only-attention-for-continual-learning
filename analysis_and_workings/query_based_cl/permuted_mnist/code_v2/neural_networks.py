...
class ERNetwork(nn.Module):
    ...
    def classify_images(self, query_x, query_y, support_x, support_y):
        ...
        row_ids = torch.arange(0, len(x), 1)
    
        mask = torch.zeros(x.shape, dtype=torch.bool).to(x.device)
        
        mask[ row_ids , query_y ] = True
        
        matched_theta_prime = x[mask].mean()
        
        
        mask = torch.ones(x.shape, dtype=torch.bool).to(x.device)
        
        mask[ row_ids , query_y ] = False
        
        unmatched_theta_prime = x[mask].mean()
    ...
    def prediction(self, data_manager_obj, query_x, query_y):
        ...
        x, delta = self.classify_images(query_x, query_y, support_x, support_y )
