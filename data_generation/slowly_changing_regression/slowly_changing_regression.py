...
def generate_problem_data(
    ...
    if num_flipping_bits > 0:
        ...
        flipping_bits = flipping_bits.repeat_interleave(flip_after, dim=0)   #we need same flipping bits for say 10000 datapoints, then next 10000 a different flipping bits
        random_bits = torch.randint(2, size=(num_data_points, num_inputs - num_flipping_bits), dtype=torch.float32) #these are bits that randomly flip every datapoint

        X = torch.cat((flipping_bits, random_bits), dim=1) #this is the input data whichcontains flipping bits and random bits in it
    else:
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(int(num_data_points/mini_batch_size))):
            ...
            Y[i*mini_batch_size:(i+1)*mini_batch_size], features = target_network.predict(x=X[i*mini_batch_size:(i+1)*mini_batch_size])

    data = X, Y, target_network
    with open( os.path.join(project_root, data_file_path) , 'wb+') as f:
        pickle.dump(data, f)
...
def main(arguments):
    ...
    parser.add_argument('-c', help="Path of the file containing the parameters of the experiment", type=str, default='env_temp_cfg/0.json')
