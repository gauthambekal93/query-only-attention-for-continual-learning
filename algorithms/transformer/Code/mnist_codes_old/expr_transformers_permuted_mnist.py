...
def expr(model_params , data_params):
    ...
    buffer_depth = 1200 #1200  took 1 hr per task!, 100 needs to be tried again.
            ...
            if start_idx < ( train_end_point ) or ( task_idx < past_task_offset ):
                ...
                with torch.no_grad():
                    ...
                    for t in range( backward_task_samples ):
                        ...
                        support_x, support_y = prev_tasks_x[t][:buffer_depth] , prev_tasks_y_one_hot[t][:buffer_depth]
                        
                        queries_x, queries_y = prev_tasks_x[t][train_end_point:], prev_tasks_y_one_hot[t][train_end_point:]
                        
                        test_z_backward, test_y_backward = buffer.create_test_data( support_x, support_y , queries_x, queries_y )
                        
                        _, backward_accuracy = transformer.test( test_z_backward, test_y_backward ) 
                    ...
                    support_x, support_y = x[:buffer_depth], y_one_hot[: buffer_depth]
                    
                    queries_x, queries_y = x[train_end_point: ], y_one_hot[train_end_point: ]
                    
                    test_z_forward, test_y_forward = buffer.create_test_data( support_x, support_y , queries_x, queries_y )
                    
                    _, forward_accuracy = transformer.test( test_z_forward, test_y_forward ) 
...
result_path_2 = os.path.join( project_root, model_params['model_dir'], 'output.pkl' )
    
with open(result_path_2, 'rb') as f:
