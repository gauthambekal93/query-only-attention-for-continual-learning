...
sys.path.append("C:/Users/gauthambekal93/Research/continual_learning/loss_of_plasticity_and_forgetting/algorithms/transformer/Code/split_image_net")
...
def expr(model_params , data_params):
    ...
    if "step_size" in model_params.keys():
    ...
    if 'linear_features' in model_params.keys():
        linear_features = model_params['linear_features']
    
    if 'attention_features' in model_params.keys():
        attention_features = model_params['attention_features']
    ...
    if "support_size" in model_params.keys():
        support_size = model_params["support_size"]
    ...
    buffer = Replay_Buffer(buffer_depth, classes_per_task, support_size, queries_per_mini_batch, dev )
    
    """We just hardcoded stepsize for testing only """
    # input_features is  128 * 3 * 3 is because we flatten the cnn output, we then multiply by 2 since we concat query_x and support_x
    relation_network = RN(input_features= 128 * 3 * 3, linear_features=linear_features, attention_features = attention_features, num_outputs=num_outputs, num_hidden_layers=num_hidden_layers, 
    ...
    zero_tensor =  torch.zeros((1, classes_per_task)).to(dev)
    ...
    for task_idx in (range(num_tasks)):
        ...
        for start_idx in tqdm(range(0, change_after, mini_batch_size)):
            ...
            if len(buffer.buffer_x)>= buffer_depth:
                ...
                data_dict_train = buffer.sample_data()
                
                train_loss, train_accuracy = relation_network.learn(data_dict_train) 
            
                train_accuracies[iter] = train_accuracy
             
                iter += 1   
      
        data_dict_val = generate_test_data(data_dict_train, image_net_val_x, image_net_val_y, label_1, label_2 , images_per_class_val, classes_per_task, zero_tensor )
        
        if  task_idx >= past_task_offset:
            
            with torch.no_grad():
                
                forward_accuracies[test_iter] = relation_network.test( data_dict_val ) 
                
                backward_accuracies[test_iter] =  relation_network.test( prev_task_data[0] ) 
...
def import_modules():
    ...
    from relation_network_split_image_net import RN
    
    global Replay_Buffer, RN
...
if __name__ == '__main__':
    ...
    model_config_path = os.path.join(project_root, "configuration_files", "split_image_net", "models","transformer", "1.json") 
