...
def generate_test_data(data_dict, image_net_val_x, image_net_val_y, label_1, label_2 , images_per_class, classes_per_task, zero_tensor ):
    
    support_x, support_y =  data_dict["support_x"], data_dict["support_y"]
    
    idx1 = torch.where(image_net_val_y == label_1)[0]  
    
    idx2 = torch.where(image_net_val_y == label_2)[0]  
    
    data_dict_val =  { "query_x":[], "query_y":[],  "support_x":[], "support_y":[] , "zero_padding": [] }
    ...
    for query_x, query_y in zip(queries_x, queries_y):
        ...
        data_dict_val["zero_padding"].append(zero_tensor)
