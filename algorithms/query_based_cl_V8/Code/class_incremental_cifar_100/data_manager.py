...
class DataManager:
     ...
     def label_remapping_test(self, labels, task_id ):
         ...
         index = []
         label_ids = self.label_ids[: self.task_id + 1].reshape(-1)
     ...
     def create_test_data(self):
        ...
        for task_id in range(self.total_tasks):
            ...
            for img, label in self.test_set:
                ...
                if label in self.label_ids[:task_id + 1 ].reshape(-1):
                    
                   self.cifar_test_x[task_id].append(img)
                     
                   self.cifar_test_y[task_id].append(label)    
            ...
            for img, label in self.train_set:
                ...
                if label in self.label_ids[:task_id + 1 ].reshape(-1):
                    ...
                    self.cifar_train_x[task_id].append(img)
                    
                    self.cifar_train_y[task_id].append(label)

        for task_id in self.cifar_train_x.keys():
            
            self.cifar_train_x[task_id] = torch.stack(self.cifar_train_x [task_id], dim = 0)
            
            unmapped_train_y = torch.tensor(self.cifar_train_y [task_id]).to(self.device)
            
            self.cifar_train_y[task_id] = self.label_remapping( unmapped_train_y, task_id )
            
            self.cifar_test_x[task_id] = torch.stack(self.cifar_test_x [task_id], dim = 0)
            
            unmapped_test_y = torch.tensor(self.cifar_test_y [task_id]).to(self.device)
            
            self.cifar_test_y[task_id] = self.label_remapping( unmapped_test_y, task_id )
