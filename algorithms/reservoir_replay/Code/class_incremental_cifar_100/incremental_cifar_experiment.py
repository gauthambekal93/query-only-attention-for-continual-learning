...
def import_modules():
    ...
    from algorithms.reservoir_replay.Code.class_incremental_cifar_100.data_manager import DataManager 
    from algorithms.reservoir_replay.Code.class_incremental_cifar_100.runner import Runner 
    from algorithms.reservoir_replay.Code.class_incremental_cifar_100.checkpoint_manager import CheckpointManager 

    from algorithms.reservoir_replay.Code.class_incremental_cifar_100.torchvision_modified_resnet import build_resnet18, kaiming_init_resnet_module
...
class IncrementalCIFARExperiment:
    ...
    def __init__(self, data_params, model_params):
        ...
        self.incoming_batch_size = data_params["incoming_batch_size"]
        ...
        self.mixing_ratio = model_params["mixing_ratio"]
    ...
    def initialize_data_manager(self):
         self.data_manager_obj = DataManager(root = ROOT, data_dir = self.data_dir, num_images_per_class = self.num_images_per_class , 
                                             initial_num_classes =self.initial_num_classes,  total_classes = self.total_classes,
                                             mixing_ratio = self.mixing_ratio, incoming_batch_size = self.incoming_batch_size , 
                                             task_lag = self.task_lag,
                                             device = self.train_context.device)
         
    
    def initialize_runner(self):
        self.runner_obj = Runner(self.data_manager_obj, self.num_epochs, incoming_batch_size = self.incoming_batch_size,
...
if __name__ == '__main__':
    ...
    model_config_path = os.path.join( ROOT, "configuration_files","cifar_100", "models", "reservoir_replay", "0.json") 
    
    data_config_path = os.path.join( ROOT, "configuration_files","cifar_100", "data", "1.json")
