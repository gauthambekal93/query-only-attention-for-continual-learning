...
class CifarData:
    ...
    def __init__(self, ROOT, data_params, model_params):
        ...
        self.label_ids = np.random.permutation(self.data_params["total_classes"])
        
        self.label_ids_flattened = self.label_ids.copy()
        
        self.label_ids = self.label_ids.reshape(-1, self.data_params["class_increase_per_task"])
    ...
    def create_cifar_data(self):
       ...
       for img, label in train_set:
           task_id = np.where(self.label_ids== label)[0][0]
    ...
    def create_task_data(self, current_task_id):
        
        if current_task_id ==0:
            
            self.task_train_x =  torch.stack( self.comp_train_x[current_task_id], dim = 0) 
            
            self.task_train_y =   torch.tensor(self.comp_train_y[current_task_id])
            
            self.task_test_x = torch.stack(  self.comp_test_x[current_task_id], dim = 0 )
            
            self.task_test_y =   torch.tensor ( self.comp_test_y[current_task_id] )
            
        else:
            self.task_train_x = torch.cat ( self.task_train_x,  torch.stack( self.comp_train_x[current_task_id], dim = 0) , dim = 0)
            
            self.task_train_y = torch.cat ( self.task_train_y,  torch.tensor(self.comp_train_y[current_task_id]), dim = 0 )
            
            self.task_test_x = torch.cat ( self.task_test_x, torch.stack(  self.comp_test_x[current_task_id], dim = 0 ), dim = 0)
            
            self.task_test_y = torch.cat ( self.task_test_y,  torch.tensor ( self.comp_test_y[current_task_id] ), dim = 0 )
