    ...
    def obtain_theta_prime(self, train_context, data_manager_obj, supports_x , supports_y, create_graph = True):
        ...
        for task_id in supports_x.keys():
            ...
            for _ in range(self.num_train_iterations):
                ...
                theta_prime = [ p_prime - train_context.inner_step_size * g  for p_prime, g in zip(theta_primes[task_id], grads) ]     
              
                flat = torch.cat([p.view(-1) for p in theta_prime])
                
                # global softmax
                probs = torch.softmax(flat, dim=0)
                
                # split back
                split_sizes = [p.numel() for p in theta_prime]
                split = torch.split(probs, split_sizes)
                
                params_softmax = [s.view_as(p) for s, p in zip(split, theta_prime)]

                theta_primes[task_id] = params_softmax   
                theta_primes[task_id] = theta_prime   
