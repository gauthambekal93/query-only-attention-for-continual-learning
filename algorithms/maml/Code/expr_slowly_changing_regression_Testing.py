# -*- coding: utf-8 -*-
"""
Created on Sat Jul 19 23:02:07 2025
...
def expr(model_params , data_params):
    ...
    num_support, num_query , num_tasks= 10, 10, 5 #100, 15, 20
    
    buffer = Replay_Buffer(buffer_size = 20000) #was 100000
    
    input_feature_size =  inputs.shape[1]
    hidden_feature_size = model_params["num_features"]
    
    maml =  MAML( inner_step_size= 0.01, outer_step_size = step_size, loss='mse', opt='adam', beta_1=0.9, beta_2=0.999, weight_decay=0.0, to_perturb =False, momentum=0, input_feature_size= input_feature_size , hidden_feature_size = hidden_feature_size).to(device)
    ...
    while i < num_data_points: 
        ...
        if  mode == "Train":
            ...
            if random.random() >0.90:
                ...
                buffer.add_data( inputs [i], outputs[i] )
            
            if ( len( buffer.dataset ) >= (buffer.buffer_size) ):     
                    begin_train =True
                    
            if begin_train and ( 1 == random.randint(1, 100) ):        
                
                support_dict, query_dict = buffer.sample_task_data(num_tasks, num_support, num_query)
            
                error = maml.learn(support_dict, query_dict)
            ...
            if begin_train :  
                ...
                supports_x_1, supports_y_1 = copy.deepcopy(supports_x), copy.deepcopy(supports_y)
                        
                queries_x_1, queries_y_1 = copy.deepcopy(queries_x), copy.deepcopy(queries_y)
                        
                forward_error = maml.test(supports_x, supports_y, queries_x, queries_y)
                ...
                for j in range(past_task_offset):
                    ...
                    backward_error = maml.test( supports_x, supports_y, queries_x, queries_y )
                    ...
                    if j ==past_task_offset-1:
                        ...
                        supports_x_2, supports_y_2 = copy.deepcopy(supports_x), copy.deepcopy(supports_y)
                        
                        queries_x_2, queries_y_2 = copy.deepcopy(queries_x), copy.deepcopy(queries_y)
                ...
                forward_effective_ranks[task_num] = maml.calculate_hessian(supports_x_1, supports_y_1, queries_x_1, queries_y_1)
                
                backward_effective_ranks[task_num] = maml.calculate_hessian(supports_x_2, supports_y_2, queries_x_2, queries_y_2)
        ...
        if ( i % datapoints_per_task ==0 ) and ( i >= datapoints_per_task):
            ...
            if train_count ==0:
                ...
                train_mse =  torch.tensor( train_errors[-train_count : ] ).mean().item()
...
if __name__ == '__main__':
    ...
    model_config_path = os.path.join(project_root, "runtime_config","models", "slowly_changing_regression", "maml","203.json")
