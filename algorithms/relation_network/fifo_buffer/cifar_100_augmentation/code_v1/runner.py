    ...
    def train(self, train_context, data_manager_obj, checkpoint_obj):
        ...
        for i in range(0, train_x.shape[0], self.num_datapoints_per_timestep ):
            ...
            if data_manager_obj.fifo_counter > 0: 
                ...
                #if len(torch.unique(data_manager_obj.fifo_y)) < data_manager_obj.classes_per_task:
                #    continue
                ...
                current_reg_loss = train_context.loss(predictions, F.one_hot(batch_y.long(), num_classes=data_manager_obj.classes_per_task ).float() )
