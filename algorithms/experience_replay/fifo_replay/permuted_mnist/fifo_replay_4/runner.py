...
class Runner:
    ...
    def forward_testing(self, train_context, batch_x, batch_y):
        ...
        accuracy = 100 * torch.mean((predictions.argmax(axis=1) == batch_y).to(torch.float32))
    ...
    def train(self, train_context, data_manager_obj, checkpoint_obj):
        ...
        #train_y_one_hot = data_manager_obj.task_train_y_one_hot[data_manager_obj.current_task_id]
        
        forward_accuracy = []
        
        test_start_idx = 58000
        
        for i in range(0, train_x.shape[0], self.num_datapoints_per_timestep ):
            
            if (i ==test_start_idx) and (data_manager_obj.current_task_id >= data_manager_obj.num_old_task_window):
                break
            ...
            #batch_y_one_hot = train_y_one_hot[ i : i + self.num_datapoints_per_timestep]  
            
            #forward_accuracy.append(  self.forward_testing(train_context, batch_x, batch_y) )
            
            train_context.net.train()
            
            #data_manager_obj.fill_buffer(  batch_x , batch_y, batch_y_one_hot)
            
            #if  data_manager_obj.buffer_counter < data_manager_obj.total_slot_ids :
            #    continue
            
            #replay_x, replay_y, replay_y_one_hot =  data_manager_obj.get_data()
        ...
        if  ( data_manager_obj.current_task_id >= data_manager_obj.num_old_task_window ):
            ...
            forward_accuracy = self.forward_testing(train_context, train_x[test_start_idx:], train_y[test_start_idx: ])
            ...
            print("task id ", data_manager_obj.current_task_id, 
                  ...
                  "Forward accuracy: ", forward_accuracy,  "Backward accuracy: ", backward_accuracy )
            
            return train_loss, train_accuracy, forward_accuracy, backward_accuracy
    ...
    def run(self, train_context, data_manager_obj, checkpoint_obj):
        ...
        while data_manager_obj.current_task_id < data_manager_obj.num_tasks: 
            ...
            train_loss, train_accuracy, forward_accuracy, backward_accuracy = self.train( train_context, data_manager_obj, checkpoint_obj)
                
            if  ( data_manager_obj.current_task_id >= data_manager_obj.num_old_task_window ) and ( data_manager_obj.current_task_id % data_manager_obj.num_old_task_window == 0 ) :
                
                checkpoint_obj.save_model_checkpoint( train_context, data_manager_obj, train_loss, data_manager_obj.current_task_id)
                
                checkpoint_obj.save_result_checkpoint(data_manager_obj, train_loss, train_accuracy, forward_accuracy, backward_accuracy)
