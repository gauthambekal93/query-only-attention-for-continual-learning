...
def import_modules():
    ...
    from algorithms.experience_replay.reservoir_replay.cifar_100_permuted.code_v1.data_manager import DataManager 
    from algorithms.experience_replay.reservoir_replay.cifar_100_permuted.code_v1.runner import Runner 
    from algorithms.experience_replay.reservoir_replay.cifar_100_permuted.code_v1.checkpoint_manager import CheckpointManager 

    from algorithms.experience_replay.reservoir_replay.cifar_100_permuted.code_v1.torchvision_modified_resnet import build_resnet18, kaiming_init_resnet_module
...
class IncrementalCIFARExperiment:
    ...
    def initialize_data_manager(self):
         self.data_manager_obj = DataManager(self.train_context.device, ROOT, self.data_dir, self.classes_per_task, self.total_classes, 
                                             self.num_old_task_window, self.num_datapoints_per_timestep, self.num_tasks, self.buffer_size, self.samples_from_buffer )
...
if __name__ == '__main__':
    ...
    model_config_path = os.path.join( ROOT, "configuration_files","cifar_100", "models", "experience_replay","reservoir_replay", "0.json") 
    
    data_config_path = os.path.join( ROOT, "configuration_files","cifar_100", "data", "2.json")
