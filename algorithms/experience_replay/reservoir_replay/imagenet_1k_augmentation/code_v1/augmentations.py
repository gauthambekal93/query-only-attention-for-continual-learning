...
def augment_batch(x: torch.Tensor, params: dict) -> torch.Tensor:
    ...
    if pad > 0:
        ...
        y_grid = torch.arange(128, device=device).view(1, 128, 1)
        x_grid = torch.arange(128, device=device).view(1, 1, 128)
        ...
        b_idx = torch.arange(B, device=device).view(B, 1, 1, 1).expand(-1, C, 128, 128)
        c_idx = torch.arange(C, device=device).view(1, C, 1, 1).expand(B, -1, 128, 128)
