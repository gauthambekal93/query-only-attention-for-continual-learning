...
class ResNet(nn.Module):
    def __init__(
        ...
        self.fc1 = nn.Linear(512, hidden_layer_size)
    ...
    def classify_images(self, query_x, support_x, support_y):
      ...
      query_x = self.relu(self.fc1(query_x))
      
      query_x = self.relu(self.fc2(query_x))
      
      query_x = self.relu(self.fc3(query_x))
      
      query_x = self.fc4(query_x)
      
      support_x = self.relu(self.fc1(support_x))
      
      support_x = self.relu(self.fc2(support_x))
      
      support_x = self.relu(self.fc3(support_x))
      
      support_x = self.fc4(support_x)
      
      support_x = torch.matmul(support_x.T, support_y.float()).T
      
      class_counts = support_y.sum(dim=0)
      
      support_x = support_x / class_counts.clamp_min(1.0).unsqueeze(1)
    ...
    def forward_prediction(self, data_manager_obj, query_x , feature_list: list = None):
