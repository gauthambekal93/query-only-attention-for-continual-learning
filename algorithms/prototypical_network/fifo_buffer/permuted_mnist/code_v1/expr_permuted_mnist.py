...
def import_modules():
    ...
    from algorithms.prototypical_network.fifo_buffer.permuted_mnist.code_v1.data_manager import DataManager 
    from algorithms.prototypical_network.fifo_buffer.permuted_mnist.code_v1.runner import Runner 
    from algorithms.prototypical_network.fifo_buffer.permuted_mnist.code_v1.checkpoint_manager import CheckpointManager 
    from algorithms.prototypical_network.fifo_buffer.permuted_mnist.code_v1.neural_networks import ERNetwork
...
class TrainContext:
    def __init__(self, input_size, embedding_features, classes_per_task, num_hidden_layers, step_size, weight_decay, total_classes):

        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

        self.net = ERNetwork(input_size=input_size,  embedding_features=embedding_features)
...
class PermutedMNISTExperiment:
    ...
    def initialize_model(self):
         self.train_context =  TrainContext(self.input_size, self.embedding_features, self.classes_per_task, self.num_hidden_layers, self.step_size, self.weight_decay, self.total_classes)
