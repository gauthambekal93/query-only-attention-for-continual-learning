...
def expr(model_params , data_params):
    ...
    ewc_network = EWC_Net(input_features= 128 * 3 * 3   , num_features=num_features, num_outputs=num_outputs, 
                          act_type=act_type, opt = opt, step_size = step_size , weight_decay= weight_decay,  momentum= momentum, 
                          beta_1=beta_1, beta_2=beta_2, loss=loss, task_datapoints = task_datapoints, classes_per_task = classes_per_task, ewc_lambda =ewc_lambda).to(dev)
    ...
    for task_idx in (range(num_tasks)):
        ...
        fisher_samples = len(train_x)
        
        for start_idx in tqdm(range(0, change_after, mini_batch_size)):
                
            train_loss, train_accuracy = ewc_network.learn( train_x [start_idx : start_idx + mini_batch_size] , 
                                                   train_y [start_idx : start_idx + mini_batch_size] , 
                                                   train_y_one_hot [start_idx: start_idx + mini_batch_size], all_ewc) 
            
            train_accuracies[iter] = train_accuracy
         
            iter += 1   
        
        ewc_t = EWC( ewc_network, train_x, train_y, fisher_samples , mini_batch_size )
        
        all_ewc.append( ewc_t  )
        ...
        if  task_idx >= past_task_offset:
            ...
            with torch.no_grad():
                ...
                forward_accuracies[test_iter] = ewc_network.test( data_dict_val ) 
                
                backward_accuracies[test_iter] =  ewc_network.test( prev_task_data[0] ) 
        ...
        if task_idx % save_after_every_n_tasks == 0:
            ...
            torch.save(ewc_network.state_dict(), model_path ) 
...
def import_modules():
    ...
    from ewc_network_split_image_net import EWC_Net, EWC
    global EWC_Net, EWC, generate_train_data, generate_test_data
...
if __name__ == '__main__':
    ...
    model_config_path = os.path.join(project_root, "configuration_files", "split_image_net", "models", "elastic_weight_compute", "0.json") 
