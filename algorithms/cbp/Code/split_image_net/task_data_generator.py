...
def generate_test_data( image_net_val_x, image_net_val_y, label_1, label_2 , images_per_class, classes_per_task ):
    ...
    test_x = torch.cat( (image_net_val_x[idx1], image_net_val_x[idx2]), dim =0)
    
    y = torch.cat( ( torch.zeros(images_per_class, dtype = torch.long), torch.ones(images_per_class, dtype = torch.long) ), dim =0)
    
    test_y = F.one_hot(y, num_classes=classes_per_task).float() 
    
    data_dict_val["test_x"] , data_dict_val["test_y"]  = test_x, test_y
