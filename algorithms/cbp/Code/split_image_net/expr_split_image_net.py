...
sys.path.append(str(BASE_DIR / "algorithms" / "cbp"/ "Code"/"split_image_net"))


#sys.path.append("C:/Users/gauthambekal93/Research/continual_learning/loss_of_plasticity_and_forgetting/common/codes")
#sys.path.append("C:/Users/gauthambekal93/Research/continual_learning/loss_of_plasticity_and_forgetting/algorithms/cbp/Code/split_image_net")
...
def expr(model_params , data_params):
    ...
    if "buffer_width" in model_params.keys():
        buffer_width = model_params["buffer_width"]
    ...
    if 'maturity_threshold' in model_params.keys():
        maturity_threshold = model_params['maturity_threshold']
    ...
    net = ConvNet2(num_classes = num_outputs , replacement_rate=replacement_rate, maturity_threshold=maturity_threshold)
    ...
    for task_idx in (range(num_tasks)):
        ...
        for start_idx in tqdm(range(0, change_after, mini_batch_size)):
            ...
            loss, accuracy = learner.learn( train_x [start_idx : start_idx + mini_batch_size] , train_y_one_hot [start_idx: start_idx + mini_batch_size]) 
        
            train_accuracies[iter] = accuracy
        ...
        if  task_idx >= past_task_offset:
            ...
            with torch.no_grad():
                ...
                forward_accuracies[test_iter] = learner.test( data_dict_val ) 
                
                backward_accuracies[test_iter] =  learner.test( prev_task_data[0] ) 
...
def import_modules():
    ...
    from conv_net2 import ConvNet2
    from convCBP import ConvCBP
    from miscellaneous import nll_accuracy, compute_matrix_rank_summaries
    #from torch.nn.functional import softmax
    
    global ConvNet, ConvNet2, ConvCBP #nll_accuracy, compute_matrix_rank_summaries, softmax
...
if __name__ == '__main__':
    ...
    model_config_path = os.path.join(project_root, "configuration_files", "split_image_net", "models", "cbp", "2.json") 
