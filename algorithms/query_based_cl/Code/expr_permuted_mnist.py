...
def expr(model_params , data_params):
    ...
    samples_per_batch = 1
    buffer_width  = x.shape[1] + 1 
    
    buffer = Replay_Buffer(buffer_depth, buffer_width, classes_per_task, support_size, samples_per_batch, dev )
    
    """We just hardcoded stepsize for testing only """
    relation_network = RN(input_size= x.shape[1]  * 2 + y_one_hot.shape[1], num_features=100, num_outputs=1, num_hidden_layers=2, 
                          act_type='relu', opt = opt, step_size = 0.0001 , weight_decay=0,  momentum=0, 
        ...
        for start_idx in tqdm(range(0, change_after, mini_batch_size)):
            ...
            if start_idx < ( train_end_point ) or ( task_idx < past_task_offset ):
                ...
                if len(buffer.buffer_x)>= buffer_depth:
                    ...
                    train_loss, train_accuracy = relation_network.learn(batch_x, batch_y) 
                ...
                with torch.no_grad():
                    ...
                    for t in range( backward_task_samples ):
                        ...
                        test_x_backward, test_y_backward = buffer.test_data_new( support_x, support_y , queries_x, queries_y )
                      
                        _, backward_accuracy = relation_network.test( test_x_backward, test_y_backward ) 
                    ...
                    test_x_forward, test_y_forward = buffer.test_data_new( support_x, support_y , queries_x, queries_y )
                    
                    _, forward_accuracy = relation_network.test( test_x_forward, test_y_forward ) 
                ...
                if (task_idx % 100 ==0) or ( task_idx == past_task_offset):
                    ...
                    forward_effective_ranks[test_iter] =  relation_network.calculate_hessian( test_x_forward[start_idx: end_idx], test_y_forward[start_idx: end_idx])
                      
                    backward_effective_ranks[test_iter] = relation_network.calculate_hessian(test_x_backward[start_idx: end_idx], test_y_backward[start_idx: end_idx])
...
def import_modules():
    ...
    from relation_network_permuted_mnist import RN
...
if __name__ == '__main__':
    ...
    model_config_path = os.path.join(project_root, "runtime_config","models", "permuted_mnist", "query_based_cl","202.json") 
