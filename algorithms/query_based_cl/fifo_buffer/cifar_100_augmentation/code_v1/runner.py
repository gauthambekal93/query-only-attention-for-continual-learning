...
class Runner:
    ...
    def train(self, train_context, data_manager_obj, checkpoint_obj):
        ...
        for i in range(0, train_x.shape[0], self.num_datapoints_per_timestep ):
            ...
            data_manager_obj.fill_fifo_buffer_1( batch_x, batch_y)
            
            if len(torch.unique(data_manager_obj.fifo_y[1])) < data_manager_obj.classes_per_task:
                continue
                         
            '''
            new_task = self.predict_task_boundary(data_manager_obj, train_context, batch_x, batch_y)
            
            if new_task: 
                
                data_manager_obj.buffer_counter = 0
                
                data_manager_obj.fill_fifo_buffer_0( batch_x, batch_y )
                
                data_manager_obj.buffer_key_counter = data_manager_obj.buffer_key_counter + 1
            '''
            
            acc = self.prequential_testing( train_context, data_manager_obj, batch_x, batch_y, fifo_id = 1)
            
            prequential_accuracy.append( acc  )
            
            #buffer_key = data_manager_obj.buffer_key_counter  % data_manager_obj.num_old_task_window

            #data_manager_obj.fill_balaced_task_buffer(batch_x, batch_y , buffer_key)
