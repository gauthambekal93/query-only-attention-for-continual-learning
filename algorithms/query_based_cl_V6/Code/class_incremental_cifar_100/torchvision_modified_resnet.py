...
class ResNet(nn.Module):
    ...
    def classify_images(self, query_x, support_x, support_y):
        ...
        support_x_shape = support_x.shape
        
        support_y_shape = support_y.shape
        
        support_x = support_x.reshape(len(query_x), -1, support_x_shape[1])
        
        support_y = support_y.reshape(len(query_x), -1, support_y_shape[1])
        
        query_x = query_x.unsqueeze(1)
        
        query_x = query_x.expand(query_shape[0], support_x.shape[1],  query_shape[1] )
        
        logits = torch.cat( (query_x, support_x, support_y), dim = -1 )
        ...
        logits = self.relu( self.fc4(logits) )
        
        logits = self.fc5(logits)
        
        predicted_label = logits.argmax(dim =1).reshape(-1)
        
        logits = logits * support_y
    ...
    def classify_images(self, query_x, support_x):
        
        logits = torch.cat( (query_x, support_x), dim = -1 )
    ...
    def forward(self, query_x: Tensor, feature_list: list = None, data_manager_obj = None, batch_y = None, unmapped_y=None,  mode ="train") -> Tensor:
        ...
        if mode=="train":
            support_x , binary_labels = data_manager_obj.get_binary_support(unmapped_y)
        
        if mode=="eval":
            support_x , support_y = data_manager_obj.get_eval_support()
            
        support_x = self.get_flattened_features( support_x , feature_list)
        
        logits = self.classify_images( query_x, support_x)
        
        logits = logits.reshape(-1)
        
        return  logits, binary_labels
        
 
    def get_predictions(self, query_x, data_manager_obj):
        
        query_x = self.get_flattened_features( query_x , feature_list = None)
        query_x = query_x.unsqueeze(0)
        query_x = query_x.expand( 1, query_x.shape[1], query_x.shape[2])  #number of supports per label
        
        prediction_scores = []
        
        for label, support_x in data_manager_obj.support_x.items():
            
            support_x = self.get_flattened_features( support_x , feature_list = None)
            
            support_x = support_x.unsqueeze(1)
            
            support_x = support_x.expand(support_x.shape[0], query_x.shape[1], support_x.shape[2])
            
            logits = self.classify_images( query_x, support_x)
            
            logits = logits.mean(dim = 0).reshape(-1)
            
            prediction_scores.append( logits )
        
        prediction_scores = torch.stack(prediction_scores, dim = 0)
        
        predictions = prediction_scores.argmax(dim =0)
