...
class ResNet(nn.Module):
    ...
    def classify_images(self, query_x, support_x, support_y):
        ...
        """
        The below line was used to get the best performance till now on(train and test agnostic setting)
    ...
    def forward(self, queries_x: Tensor, feature_list: list = None, data_manager_obj = None, batch_y = None, task_id = 0, mode = "train") -> Tensor:
        ...
        queries_x = self.get_flattened_features( queries_x , feature_list)
        
        predictions = torch.empty ( len(queries_x),  data_manager_obj.num_neg_support_labels + 1  ).to(queries_x.device)
          
        data_manager_obj.get_train_support(batch_y)
        
        supports_x =  data_manager_obj.supports_x[unique_labels]
        
        supports_x = supports_x.reshape(-1, supports_x.shape[2], supports_x.shape[3], supports_x.shape[4])
        
        supports_y =  data_manager_obj.supports_y[unique_labels]
            
        supports_x = self.get_flattened_features( supports_x , feature_list)
        
        try:
            supports_x = supports_x.reshape(len(unique_labels), data_manager_obj.total_sample_supports, supports_x.shape[1])
        except:
            print("stop")
            print("stop")
        for i, label in enumerate(unique_labels):
            
            mask = torch.isin(batch_y, label)
            
            ids = torch.nonzero(mask, as_tuple=True)[0]
            
            predictions[ids] = self.classify_images( queries_x[mask], supports_x[i], supports_y[i])
        
        return predictions
    '''
    
    '''
    def forward(self, query_x: Tensor, feature_list: list = None, data_manager_obj = None, batch_y = None, task_id = 0, mode = "train") -> Tensor:
        ...
        if mode=="train":
            data_manager_obj.get_train_support()

            
        if mode =="eval":
            support_x = data_manager_obj.get_eval_support()
    ...
    def forward_eval(self, query_x: Tensor, feature_list: list = None, data_manager_obj = None, batch_y = None, task_id = 0, query_id = 0) -> Tensor:
           ...
           support_x , support_y = data_manager_obj.get_eval_support () #data_manager_obj.eval_support_x, data_manager_obj.eval_support_y
    ...
    #def forward(self, queries_x: Tensor, feature_list: list = None, data_manager_obj = None, batch_y = None, task_id = 0, mode = "train") -> Tensor:
    def forward(self, query_x: Tensor, query_y: Tensor, support_x: Tensor, support_y: Tensor, data_manager_obj = None, task_id = 0, mode = "train", feature_list: list = None) -> Tensor:
        ...
        support_x = data_manager_obj.augment_batch(support_x)
