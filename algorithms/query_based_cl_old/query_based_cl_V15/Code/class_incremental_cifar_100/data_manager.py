...
class DataManager:
     ...
     def __init__(self, root, data_dir, num_images_per_class, initial_num_classes, class_increase_per_task, total_classes, num_labels_previous_task, num_data_points_current_task , num_support_per_label, device):
         ...
         self.buffer = torch.empty(self.total_classes, 20, 3, 32, 32).to(self.device) #{ i: torch.empty( 0 ) for i in range(self.total_classes)}
         ...
         self.test_support_y = torch.cat([ torch.tensor([j] *20) for j in range(self.total_classes) ])
     ...
     def get_train_support(self):
            ...
            rand_rows = torch.randperm(100).to(self.device)
            
            rand_cols = torch.randint(0, 20, (100,)).to(self.device)
            
            support_x = self.buffer[rand_rows, rand_cols]
            
            support_y = F.one_hot(rand_rows, num_classes=self.total_classes).to(self.device, dtype=torch.float32)
     ...
     def get_train_support(self):
               ...
               rand_rows = rand_rows.unsqueeze(dim = 1)
               rand_rows = rand_rows.expand(100, 20 )
               
               rand_cols = torch.randperm(20).to(self.device)
               rand_cols = rand_cols.unsqueeze(dim = 0)
               rand_cols = rand_cols.expand(100, 20 )
               
               
               support_x = self.buffer[rand_rows, rand_cols]
               
               support_x = support_x.reshape(-1, support_x.shape[2], support_x.shape[3], support_x.shape[4])
               support_y = torch.cat( [ torch.tensor( [label] * 20 ) for label in range(self.total_classes) ],dim = 0)
               
               support_y = F.one_hot( support_y, num_classes = self.total_classes  ).to(self.device)  
     ...
     '''
     def get_train_support(self, batch_y):

               #import time
           
               #start = time.time()
               #start0 = time.time() .,mmmmmmmmmmmmmmmmm

               neg_col_ids = torch.randperm(20)[:self.num_neg_support_per_label]
               pos_col_ids = torch.randperm(20)[:self.num_pos_support_per_label]
               #print("1: ",time.time() - start0)
               for label in batch_y.unique():
                   #start1 = time.time()
                   rand_rows = torch.randperm(100).to(self.device)
                   #print(time.time() - start1)
                   #start2 = time.time()
                   mask = torch.isin(rand_rows, label)
                   other_labels = rand_rows[~mask][:self.num_neg_support_labels]
                   #print("2: ",time.time() - start2)
                   #THE BELOW LINE NEEDS TO BE CHECKED CORRECTLY!!!
                   #start3 = time.time()
                   non_matching_support  = self.buffer[other_labels][:, neg_col_ids]
                   non_matching_support  = non_matching_support.reshape(-1, non_matching_support.shape[2], 
                                                                        non_matching_support.shape[3], 
                                                                        non_matching_support.shape[4])
                   #print("3: ",time.time() - start3)
                   #start4 = time.time()
                   matching_support  = self.buffer[label, pos_col_ids]
                   #print("4: ",time.time() - start4)
                   #start5 = time.time()
                   self.supports_x[label] = torch.cat( (matching_support, non_matching_support) , dim = 0) 
                   self.supports_y[label] = self.support_y
    
                   #print("5: ",time.time() - start5)
     ...
     def create_unique_labels(self):
         ...
         self.unique_labels = torch.randperm(self.total_classes)[:10].to(self.device)
     ...
     def get_data(self):
         ...
         num_queries_per_label, num_support_per_label = 2, 5
