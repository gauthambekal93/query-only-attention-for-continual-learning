...
class DataManager:
     ...
     def __init__(self, root, data_dir, num_images_per_class, initial_num_classes, class_increase_per_task, total_classes, num_labels_previous_task, num_data_points_current_task , num_support_per_label, device):
         ...
         self.class_increase_per_task = 10
         ...
         self.is_filled =  torch.tensor ( [0] * self.total_classes )
         ...
         self.task_ids = torch.randperm(100)
     ...
     def create_unique_labels_test(self):
         
         self.unique_labels_test = self.task_ids[:self.current_task_id*self.class_increase_per_task  + self.class_increase_per_task].to(self.device)
         
         self.mapping_test = {label.item(): i for i, label in enumerate(self.unique_labels_test)}
         
     def remap_test_labels(self):
         
         mapped_task_test_y = {}
         
         for task_id, labels in self.task_test_y.items():
             
             mapped_task_test_y[task_id] = torch.tensor([self.mapping_test[label.item()] for label in labels]).to(self.device)
        
         return mapped_task_test_y
     
         
     def get_eval_support(self):
             
             support_x = self.buffer[self.unique_labels_test, :self.buffer.shape[1]]
             
             support_x = support_x.reshape(-1, support_x.shape[2], support_x.shape[3], support_x.shape[4])
             
             test_support_y = torch.cat([ torch.tensor([j] *self.buffer.shape[1]) for j in range(len(self.unique_labels_test)) ])
                 
             test_support_y = F.one_hot( test_support_y, num_classes = len(self.unique_labels_test)   ).to(self.device)  
             
             return support_x, test_support_y
         
                 
     def create_unique_labels_train(self):
         
         available_labels = torch.where(self.is_filled > 0)[0] 
         
         select_ids = torch.randperm(len(available_labels))[: self.class_increase_per_task ]
         
         self.unique_labels = available_labels[select_ids].to(self.device)
     ...
     def create_task_data(self):
             ...
             task_labels = self.task_ids[self.current_task_id* self.class_increase_per_task: self.current_task_id*self.class_increase_per_task  + self.class_increase_per_task].to(self.device)
