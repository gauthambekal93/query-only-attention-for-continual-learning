...
class Runner:
    ...
    def test_network(self, train_context, data_manager_obj, checkpoint_obj):
         ...
         with torch.no_grad():
             ...
             data_manager_obj.create_unique_labels_test()
             
             mapped_task_test_y = data_manager_obj.remap_test_labels()
             
             for task_id in data_manager_obj.task_test_x.keys():
                 
                 batch_x, batch_y_mapped = data_manager_obj.task_test_x[ task_id ], mapped_task_test_y[ task_id ]
                 
                 predictions = train_context.net.forward_eval(batch_x, data_manager_obj = data_manager_obj, batch_y = batch_y_mapped, task_id = task_id).to(train_context.device) 
                 
                 accuracy = 100* torch.mean((predictions.argmax(axis=1) == batch_y_mapped).to(torch.float32)).item()
                 
                 avg_acc += accuracy
                 
                 sub_task_accuracies[task_id] = accuracy
             
                 print("Test task id", task_id, "Accuracy ", accuracy)
             
         accuracy =  avg_acc / ( len(data_manager_obj.task_test_x.keys() ) ) 
          
         print("Aggregate test task accuracy: ", accuracy  )
    ...
    def train(self, train_context, data_manager_obj, checkpoint_obj):
        ...
        total_epochs, number_of_batches  = 200, train_x.shape[0] // self.train_batch_size
        
        for epoch in range(total_epochs):
            
            counter, train_accuracy, train_loss  = 0, torch.empty(  number_of_batches ), torch.empty(  number_of_batches )
            ...
            for i in range(0, train_x.shape[0], self.train_batch_size ):
                ...
                if sum(data_manager_obj.is_filled) < 5: #100:
                
                    continue
                
                data_manager_obj.create_unique_labels_train()
                ...
                train_accuracy[counter] = 100 * torch.mean((predictions.argmax(axis=1) == query_y).to(torch.float32))
                
                train_loss[counter] = current_reg_loss
                
                counter += 1
            
            if epoch % 10 == 0 or epoch == total_epochs - 1:
                
                 avg_loss, avg_accuracy  = torch.mean(  train_loss ).item(), torch.mean(  train_accuracy ).item()
                 
                 print("task id ", data_manager_obj.current_task_id, "Epoch ", epoch, "Train loss ",  avg_loss , "Train accuracy ", avg_accuracy)
                    
                 _, _ = self.test_network( train_context, data_manager_obj, checkpoint_obj)
