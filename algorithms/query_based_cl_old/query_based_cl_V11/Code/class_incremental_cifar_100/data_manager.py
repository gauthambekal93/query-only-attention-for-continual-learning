...
class DataManager:
     ...
     def __init__(self, root, data_dir, num_images_per_class, initial_num_classes, class_increase_per_task, total_classes, num_labels_previous_task, num_data_points_current_task , num_support_per_label, device):
         ...
         #self.global_data = { i: torch.empty( (0, 0, 0, 0) ) for i in range(self.total_classes)}
         
         self.global_data = { i: torch.empty( 0 ) for i in range(self.total_classes)}
         
         self.eval_support_x = None
     ...
     def label_remapping(self, labels ):
         ...
         self.classes[self.unique_labels] = torch.arange(len(self.unique_labels), device = self.device)

         # Map labels → indices
         index = self.classes[labels]
     ...
     def create_global_data(self, X, Y):
         ...
         for label in self.unique_labels:
             ...
             if len(self.global_data[label.item()])==0:  #was <=20
     ...
     def create_task_data(self):
             ...
             self.task_train_y[self.current_task_id] = self.label_remapping(self.train_y [ mask ][train_rand_ids]  )
             
             """Fill in with seen data to a buffer for creating the support for evaluvation """
             
             
             self.is_filled = all( [ True if len(imgs) >=20 else False for imgs in self.global_data.values()  ] )
             
             if self.is_filled is False:
                 
                 self.create_global_data( self.task_train_x[self.current_task_id] , self.train_y [ mask ][train_rand_ids]  )
            
             else:
                 
                 if self.eval_support_x is None:
                     
                     self.eval_support_x = torch.cat( [ img for img in self.global_data.values() ] , dim = 0)
                 
                     self.eval_support_y = torch.cat( [ torch.tensor( [label] * 20 ) for label in self.global_data.keys()],dim = 0)
                     
                     self.eval_support_y = F.one_hot( self.eval_support_y, num_classes = self.total_classes  ).to(self.device)
     ...
     def get_train_support(self, task_id):
