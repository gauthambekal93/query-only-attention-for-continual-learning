...
class Runner:
    ...
    def test_network(self, train_context, data_manager_obj, checkpoint_obj):
         ...
         with torch.no_grad():
             ...
             for task_id in data_manager_obj.task_test_x.keys():
                 ...
                 top5 = predictions.topk(5, dim=1).indices
                 
                 top5_acc = (top5 == batch_y.unsqueeze(1)).any(dim=1).float().mean().item()
                 
                 #print("Test Task id ", task_id, "Accuracy ", accuracy, "top5_acc ", top5_acc)
    ...
    def test_network(self, train_context, data_manager_obj, checkpoint_obj):
             ...
             print("Task id ", task_id, "Test task accuracy ", 100 * accuracy )
    '''
    ...
    def run(self, train_context, data_manager_obj, checkpoint_obj):
        ...
        checkpoint = torch.load(r"C:/Users/gauthambekal93/Research/query-only-attention-for-continual-learning/results/cifar_100/query_based_cl_V10/0/0/model_5_way_classification_20_supports_x_3.pkl"  ,  map_location = train_context.device)
        
        #checkpoint = torch.load(r"C:/Users/gauthambekal93/Research/query-only-attention-for-continual-learning/results/cifar_100/query_based_cl_V10/0/0/model_5_way_classification_20_supports_x.pkl"  ,  map_location = train_context.device)
        ...
        while data_manager_obj.current_task_id < 50000: #data_manager_obj.total_tasks:
            ...
            if data_manager_obj.eval_support_x is not None:
