...
class DataManager:
     ...
     def create_task_data(self):
         ...
         self.task_train_labels = torch.cat( [ self.comp_train_y[task_id] for task_id in range(self.current_task_id + 1)]).to(self.device)
     ...
     def get_support_indices(self):
         ...
         task_labels =  self.label_ids[:self.current_task_id + 1].reshape(-1)
         
         rand_idx = torch.randperm( 500 ) [: self.buffer_size_per_label ]
         
         support_indices = []
         for i, task_label in enumerate(task_labels):
             #support_index = (self.comp_train_y[self.current_task_id] == task_label).nonzero(as_tuple=True)[0][rand_idx]
             support_index = (self.task_train_labels == task_label).nonzero(as_tuple=True)[0][rand_idx]
