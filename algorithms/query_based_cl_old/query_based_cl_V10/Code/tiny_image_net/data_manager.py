...
class DataManager:
     ...
     def __init__(self, root, data_dir, num_images_per_class, initial_num_classes, total_classes, num_support_per_label, task_lag, device):
     ...
     def create_global_data(self, X, Y):
         ...
         for label in self.unique_task_labels:
             ...
             if len(self.global_data[label.item()])==0:  
                     ...
                     self.global_data[label.item()] = X[mask][:self.num_support_per_label]
     ...
     def create_task_data(self):
             ...
             val_mask = torch.isin( self.val_y,  self.unique_task_labels)
             
             train_rand_ids = torch.randperm(self.train_y [ train_mask ] .shape[0])
             ...
             """Create val data """
             self.task_val_x[self.current_task_id] = self.val_x [val_mask]
             
             self.task_unmapped_val_y[self.current_task_id] = self.val_y [ val_mask ]
