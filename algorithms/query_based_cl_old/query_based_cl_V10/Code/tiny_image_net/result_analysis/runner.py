...
class Runner:
    ...
    def evaluvate_network_2(self, train_context, data_manager_obj, checkpoint_obj, eval_type):
         ...
         with torch.no_grad():
           ...
           task_id = data_manager_obj.current_task_id - data_manager_obj.task_lag   
           count = 0
           for i in range(task_id, data_manager_obj.current_task_id ):
                
                if eval_type =="task validation":    
                    batch_x, batch_y = data_manager_obj.task_val_x[ i ], data_manager_obj.task_mapped_val_y[ i ]
                    accuracy_type = "Task Validation Accuracy"
                
                if eval_type =="task test":    
                    batch_x, batch_y = data_manager_obj.task_test_x[ i ], data_manager_obj.task_mapped_test_y[ i ]
                    accuracy_type = "Task Test Accuracy"
                
                if eval_type =="global validation":    
                    batch_x, batch_y = data_manager_obj.task_val_x[ i ], data_manager_obj.task_unmapped_val_y[ i ]
                    accuracy_type = "Global Validation Accuracy"
                    
                if eval_type =="global test":    
                     batch_x, batch_y = data_manager_obj.task_test_x[ i ], data_manager_obj.task_unmapped_test_y[ i ]
                     accuracy_type = "GLOBAL Test Accuracy: "
