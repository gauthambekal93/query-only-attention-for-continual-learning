...
class ResNet(nn.Module):
    ...
    def forward(self, query_x: Tensor, feature_list: list = None, data_manager_obj = None, batch_y = None, task_id = 0, eval_type = "train") -> Tensor:
        ...
        if eval_type =="train":
            ...
            support_x, support_y  = data_manager_obj.get_train_support(task_id)
        ...
        if (eval_type =="task validation")  or (eval_type == "task test"):
            
            support_x, support_y  = data_manager_obj.local_eval_support_x[task_id], data_manager_obj.local_eval_support_y[task_id]
            
            support_x = self.get_flattened_features( support_x , feature_list)
        
            predictions = self.classify_images( query_x, support_x, support_y)
        
        if ( eval_type == "global validation") or ( eval_type == "global test"):

            support_x , support_y, label_history = data_manager_obj.get_global_supports()
            
            support_x = self.get_flattened_features( support_x , feature_list)
            
            predictions = self.classify_images( query_x, support_x, support_y)

            mask = torch.zeros( predictions.shape[1], dtype=torch.bool, device=predictions.device)
            
            mask[label_history] = True
