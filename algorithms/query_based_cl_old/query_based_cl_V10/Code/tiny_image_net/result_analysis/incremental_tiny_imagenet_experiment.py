...
def import_modules():
    ...
    from algorithms.query_based_cl_V10.Code.tiny_image_net.result_analysis.data_manager import DataManager 
    from algorithms.query_based_cl_V10.Code.tiny_image_net.result_analysis.runner import Runner 
    from algorithms.query_based_cl_V10.Code.tiny_image_net.result_analysis.checkpoint_manager import CheckpointManager 

    from algorithms.query_based_cl_V10.Code.tiny_image_net.result_analysis.torchvision_modified_resnet import build_resnet18, kaiming_init_resnet_module
...
class Incremental_Tiny_Imagenet_Experiment:
    ...
    def __init__(self, data_params, model_params):
        ...
        self.task_lag = data_params["task_lag"]
        
        self.eval_frequency = data_params["eval_frequency"]
    ...
    def initialize_data_manager(self):
         self.data_manager_obj = DataManager(root = ROOT, data_dir = self.data_dir, num_images_per_class = self.num_images_per_class , 
                                             ...
                                             num_support_per_label = self.num_support_per_label, 
                                             task_lag  = self.task_lag , device = self.train_context.device)
         
    
    def initialize_runner(self):
        self.runner_obj = Runner(self.data_manager_obj, train_batch_size = self.train_batch_size, 
                                 val_batch_size = self.val_batch_size, test_batch_size = self.test_batch_size, eval_frequency = self.eval_frequency)
...
def main(arguments):
   ...
   exp_obj = Incremental_Tiny_Imagenet_Experiment(data_params, model_params)
...
if __name__ == '__main__':
    ...
    model_config_path = os.path.join( ROOT, "configuration_files","tiny_image_net", "models", "query_based_cl_V10", "0.json") 
    
    data_config_path = os.path.join( ROOT, "configuration_files","tiny_image_net", "data", "0.json")
