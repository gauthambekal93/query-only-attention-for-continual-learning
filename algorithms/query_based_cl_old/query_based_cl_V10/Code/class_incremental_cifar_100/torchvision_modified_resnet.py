...
class ResNet(nn.Module):
    ...
    def forward(self, query_x: Tensor, feature_list: list = None, data_manager_obj = None, batch_y = None, task_id = 0, data_type = "train") -> Tensor:
        ...
        if data_type =="train":
            ...
            support_x, support_y  = data_manager_obj.eval_support_x[task_id], data_manager_obj.eval_support_y[task_id]
            
            support_x = torch.cat( [data_manager_obj.augment_batch(support_x) for _ in range(3)] , dim =0)
            
            support_y = torch.cat( [support_y for _ in range(3)] , dim =0) 
