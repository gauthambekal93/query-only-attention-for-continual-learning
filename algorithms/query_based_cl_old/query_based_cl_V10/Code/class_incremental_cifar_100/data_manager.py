...
class DataManager:
     ...
     def __init__(self, root, data_dir, num_images_per_class, initial_num_classes, class_increase_per_task, total_classes, num_labels_previous_task, num_data_points_current_task , num_support_per_label, device):
         ...
         self.eval_support_x, self.eval_support_y = {}, {}
     ...
     def label_remapping(self, labels ):
         ...
         classes[self.unique_labels] = torch.arange(len(self.unique_labels), device = self.device)
     ...
     def create_eval_support(self):
         ...
         for label in torch.unique(self.task_train_y[self.current_task_id]):
             
             support_indices = (self.task_train_y[self.current_task_id] == label).nonzero(as_tuple=True)[0]
             ...
             support_y.append(self.task_train_y[self.current_task_id][support_indices])
     
         self.eval_support_x[self.current_task_id]  = torch.cat(support_x, dim=0)
         
         support_y = torch.cat(support_y, dim=0)
        
         self.eval_support_y[self.current_task_id] = F.one_hot(support_y, num_classes=self.initial_num_classes).to(self.device, dtype=torch.float32)
     ...
     def create_task_data(self):
             ...
             self.task_val_y[self.current_task_id] = self.label_remapping( self.train_y [ mask ][val_rand_ids]   ) 
             ...
             self.task_test_y[self.current_task_id] = self.label_remapping( self.test_y [ mask ] )
     ...
     def delete_data(self):
         ...
         del self.eval_support_x[self.current_task_id - 20]
         
         del self.eval_support_y[self.current_task_id - 20]
