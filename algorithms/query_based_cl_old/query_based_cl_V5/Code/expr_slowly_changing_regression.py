...
def expr(model_params , data_params):
    ...
    """Buffer and model related configurations """
    num_support = 100   

    buffer = Replay_Buffer()    
    
    input_feature_size =  inputs.shape[1]  * 2
      
    relation_network =  RN( step_size= step_size, loss='mse', opt='adam', beta_1=0.9, beta_2=0.999, weight_decay=0.0, to_perturb =False, momentum=0, input_feature_size= input_feature_size ).to(device)
    ...
    while i < num_data_points: 
        ...
        if  mode == "Train":
            ...
            if i >= num_support :
                ...
                x, y = buffer.create_train_data(inputs[i-num_support: i+1], outputs[i-num_support: i+1], num_support  )
            
                train_errors.append( relation_network.learn( x , y ) ) 
            ...
            with torch.no_grad():
                ...
                x, y = buffer.create_test_data(supports_x, supports_y , queries_x, queries_y, num_support )
                
                x_copy, y_copy  = copy.deepcopy(x), copy.deepcopy(y)
                
                forward_error = relation_network.test(x, y )
                ...
                for j in range(past_task_offset):
                    ...
                    backward_error = relation_network.test( x, y )
            ...
            forward_effective_ranks[task_num] = relation_network.calculate_hessian(x_copy, y_copy)
            
            backward_effective_ranks[task_num] = relation_network.calculate_hessian(x_copy_2, y_copy_2)
...
def import_modules():
    ...
    from relation_network import RN
...
if __name__ == '__main__':
    ...
    model_config_path = os.path.join(project_root, "runtime_config","models", "slowly_changing_regression", "query_based_cl","201.json")
