# -*- coding: utf-8 -*-
"""
Created on Fri Dec 19 11:15:04 2025

@author: gauthambekal93
"""


import torch
from tqdm import tqdm
import time
import torch.nn.functional as F

class Runner:
    
    def __init__(self,  data_manager_obj, num_epochs, train_batch_size, val_batch_size, test_batch_size):
        
        self.num_epochs = num_epochs
        self.epochs_per_task = int( self.num_epochs / data_manager_obj.total_tasks ) #epochs_per_task
        self.train_batch_size = train_batch_size
        self.val_batch_size = val_batch_size
        self.test_batch_size = test_batch_size
      
    
    def set_lr(self, epoch, train_context):
            """ Changes the learning rate of the optimizer according to the current epoch of the task """
            current_stepsize = None
            
            if epoch  == 0:
                current_stepsize = train_context.step_size

            elif epoch == 200:
                current_stepsize = round(train_context.step_size * 0.2, 5)
            
            elif epoch == 800:
                current_stepsize = round(train_context.step_size * (0.2 ** 2), 5)
            
            elif epoch == 1600:
                current_stepsize = round(train_context.step_size * (0.2 ** 3), 5)

            if current_stepsize is not None:
                for g in train_context.optim.param_groups:
                    g['lr'] = current_stepsize
                    

    def test_network(self, epoch, train_context, data_manager_obj, checkpoint_obj):
         
         train_context.net.eval()
         
         avg_acc = 0.0
         sub_task_accuracies = {}

         with torch.no_grad():
             
             for task_id in data_manager_obj.task_test_x.keys():
                 
                 batch_x, batch_y = data_manager_obj.task_test_x[ task_id ], data_manager_obj.task_test_y[ task_id ]
                 
                 predictions = train_context.net.forward(batch_x, data_manager_obj = data_manager_obj, batch_y = batch_y, mode ="eval").to(train_context.device) 
                 
                 accuracy = torch.mean((predictions.argmax(axis=1) == batch_y).to(torch.float32)).item()
                 
                 avg_acc += accuracy
                 
                 sub_task_accuracies[task_id] = accuracy
               
         checkpoint_obj.summarize_test( avg_acc / (data_manager_obj.current_task_id + 1 ) , sub_task_accuracies, data_manager_obj.current_task_id, data_manager_obj.current_num_classes )
         
         print("Test task accuracy: ", 100 * (avg_acc / (data_manager_obj.current_task_id + 1 ) )  )
         
         print("Test sub-task accuracy: ", { task_id: 100*accuracy for task_id, accuracy in sub_task_accuracies.items() }  )
     
                     
    def val_network(self, epoch, train_context, data_manager_obj, checkpoint_obj):
         
         train_context.net.eval()

         avg_acc = 0.0
         sub_task_accuracies = {}
         
         with torch.no_grad():
             
             for task_id in data_manager_obj.task_val_x.keys():
                 
                 batch_x, batch_y = data_manager_obj.task_val_x[ task_id ], data_manager_obj.task_val_y[ task_id ]
                 
                 predictions = train_context.net.forward(batch_x, data_manager_obj = data_manager_obj, batch_y = batch_y, mode ="eval").to(train_context.device) 
                 
                 accuracy = torch.mean((predictions.argmax(axis=1) == batch_y).to(torch.float32)).item()
                 
                 avg_acc += accuracy
                 
                 sub_task_accuracies[task_id] = accuracy
                 
         print("Validation task accuracy ", 100 * (avg_acc / (data_manager_obj.current_task_id + 1 ) ) )
         
         print("Validation sub-task accuracy: ", { task_id: 100*accuracy for task_id, accuracy in sub_task_accuracies.items() }  )
    
    
    def train(self, train_context, data_manager_obj, checkpoint_obj):

        """train model """
        #start = time.perf_counter()
        
        train_context.net.train()
        
        actual = torch.tensor([[1,0]], dtype = torch.float32)
        actual = actual.expand(self.train_batch_size, actual.shape[1]).to( train_context.device )
        
        for epoch in tqdm( range(self.epochs_per_task)):
            
            if epoch ==0  or epoch == self.epochs_per_task -1 :
                print("task id ", data_manager_obj.current_task_id, "Epoch ", epoch)
                #self.val_network(epoch, train_context, data_manager_obj, checkpoint_obj) 
                print("===========================================================================================")
                 
            #print("Epoch ", epoch)
            self.set_lr(epoch, train_context)
            
            rand_idx = torch.randperm(len(data_manager_obj.task_train_y)) 
            
            no_of_batches = len(rand_idx) // self.train_batch_size
            
            for batch_no in range( no_of_batches ): 
                
                #start = time.perf_counter()
                
                start_id , end_id = self.train_batch_size  * batch_no,  self.train_batch_size  * (batch_no +1)
                
                batch_ids = rand_idx[start_id: end_id ]
                
                batch_x, batch_y = data_manager_obj.task_train_x[batch_ids], data_manager_obj.task_train_y[batch_ids]
                
                batch_x = data_manager_obj.augment_batch(batch_x)
            
                
                for param in train_context.net.parameters(): 
                    param.grad = None   # apparently faster than optim.zero_grad()
                
                unmapped_y = data_manager_obj.unmapped_train_y[batch_ids]
               
                logits, binary_labels = train_context.net.forward( batch_x, data_manager_obj = data_manager_obj, batch_y = batch_y, unmapped_y = unmapped_y, mode = "train")
                
                predictions = train_context.net.get_predictions( query_x = batch_x, data_manager_obj = data_manager_obj)
                
                current_reg_loss = train_context.loss(logits, binary_labels)
                
                current_reg_loss.backward()
                
                train_context.optim.step()

                checkpoint_obj.running_accuracy += torch.mean((predictions == batch_y).to(torch.float32)).detach()
                
                checkpoint_obj.running_loss += current_reg_loss.detach()
                              
                """save checkpoints """
                #if (batch_no + 1) % 16 ==0: 
                #    checkpoint_obj.summarize_train()
            
            checkpoint_obj.summarize_train()
           
        """obtain performance """
        #self.test_network(epoch, train_context, data_manager_obj, checkpoint_obj)
        
        #print("t1", time.perf_counter() - start   )         
        checkpoint_obj.save_experiment_checkpoint(train_context, data_manager_obj)
        
        
        
    def run(self, train_context, data_manager_obj, checkpoint_obj):
        
        while data_manager_obj.current_task_id < data_manager_obj.total_tasks:
            
            data_manager_obj.create_train_data()
            
            data_manager_obj.fill_replay_buffer()
            
            data_manager_obj.create_eval_data()
            
            data_manager_obj.create_prediction_support()
            
            self.train( train_context, data_manager_obj, checkpoint_obj)
            
            data_manager_obj.current_num_classes += data_manager_obj.class_increase_per_task
            
            #data_manager_obj.fill_replay_buffer()
            
            data_manager_obj.current_task_id += 1
        
        
