...
class DataManager:
     ...
     def __init__(self, root, data_dir, num_images_per_class, initial_num_classes, class_increase_per_task, total_classes, buffer_size_per_label, device):
         
         self.current_num_classes = initial_num_classes
         self.class_increase_per_task = class_increase_per_task
         self.total_classes = total_classes
         self.current_task_id = 0
         self.buffer_size_per_label = buffer_size_per_label
         ...
         self.replay_buffer = { label: Queue(maxsize=self.buffer_size_per_label) for label in range(self.total_classes) }
     ...
     def create_train_data(self):
         ...
         num_classes_without_previous_tasks = self.label_ids.shape[1]
             
         self.task_train_y = F.one_hot( torch.tensor(index), num_classes = num_classes_without_previous_tasks ).to(self.device, dtype=torch.float32) 
         
         #self.task_train_labels = torch.cat( [ self.comp_train_y[self.current_task_id] ]).to(self.device) 
     ...
     def create_test_data(self):
          ...
          for label in torch.cat( [ self.comp_test_y[task_id] for task_id in range(self.current_task_id + 1) ]):
              
              index .append( (self.label_ids[:self.current_task_id + 1].reshape(-1) == label).nonzero(as_tuple=True)[0].item() )
              
              self.label_id_mapping [label.item()]  = index[-1]
              
          self.task_test_y = F.one_hot( torch.tensor(index), num_classes =  self.current_num_classes).to(self.device, dtype=torch.float32) 
          
          
     def get_train_support(self):
         
         rand_idx = torch.randperm( 500 ) [: self.buffer_size_per_label ]
         
         train_labels =  self.label_ids[self.current_task_id ]
         
         support_indices = []

         for i, label in enumerate(train_labels):

             #support_index = (self.task_train_labels == label).nonzero(as_tuple=True)[0][rand_idx]
             support_index = ( self.comp_train_y[self.current_task_id] == label).nonzero(as_tuple=True)[0][rand_idx]

             support_indices.append(  support_index )
             
         support_indices = torch.cat(support_indices )     
         
         support_x = self.task_train_x [support_indices]
         
         support_y = self.task_train_y [support_indices]
         
         support_x = self.augment_batch(support_x)
         
         if self.current_task_id not in self.test_support_x  :
             self.test_support_x[self.current_task_id] = support_x
         
         if self.current_task_id not in self.test_support_y:
             self.test_support_y[self.current_task_id] = support_y
             
         return support_x , support_y


     '''     
     def create_test_support(self): 
       
        for task_id in self.comp_train_y.keys():
            
            rand_idx = torch.randperm( 500 ) [: self.buffer_size_per_label ]
        
            test_labels =  self.label_ids[ task_id ]
            
            support_indices = []
            
            for i, label in enumerate(test_labels):
                
                support_index = (self.comp_train_y[task_id] == label).nonzero(as_tuple=True)[0][rand_idx]
                
                support_indices.append(  support_index )
    
            support_indices = torch.cat(support_indices )  
            
            self.test_support_x[task_id] = self.comp_train_x[task_id] [support_indices].to(self.device, dtype=torch.float32) 
            
            self.test_support_y[task_id] 
    
            index = [] 
            
            for label in self.comp_train_y[task_id]:
                
                index.append( (self.label_ids[task_id ] == label).nonzero(as_tuple=True)[0].item() )
            
            num_classes_without_previous_tasks = self.label_ids.shape[1]
                
            task_test_y = F.one_hot( torch.tensor(index), num_classes = num_classes_without_previous_tasks ).to(self.device, dtype=torch.float32) 
     '''    
    
     '''
     def create_test_support_old(self):
              
              rand_idx = torch.randperm( 500 ) [: self.buffer_size_per_label ]
              
              for task_id in self.comp_train_y.keys():
                  
                  task_labels = self.label_ids[task_id]
                  ...
                  #F.one_hot( torch.tensor(index), num_classes = num_classes_without_previous_tasks ).to(self.device, dtype=torch.float32) 
                  
                  self.test_support_y[task_id] = self.comp_train_y[task_id] [support_indices].to(self.device, dtype=torch.float32) 
