...
class ResNet(nn.Module):
    ...
    def forward(self, query_x: Tensor, feature_list: list = None, data_manager_obj = None, mode = "train", batch_y = None) -> Tensor:
        ...
        if mode == "train":        
            support_x , support_y = data_manager_obj.get_train_support()
        ...
        if mode == "test": 
            ...
            for task_id in range(data_manager_obj.current_task_id + 1):
                
                support_x, support_y = data_manager_obj.test_support_x[task_id], data_manager_obj.test_support_y[task_id]
                
                support_x = self.get_flattened_features( support_x , feature_list)
                
                classification_decision[task_id] = self.classify_images( query_x, support_x, support_y)
            
            if data_manager_obj.current_task_id == 0:
                
                selected_col_ids = classification_decision[0].argmax(dim = 1)
                
                predictions = F.one_hot( selected_col_ids, num_classes = data_manager_obj.current_num_classes ) #.to(self.device) 
                
                return predictions
            
            else:
                
                predictions = torch.zeros( ( len(support_y), data_manager_obj.current_num_classes) )  #we can replace support_y by query_x for simplicity 
                
                equal_probs = torch.ones(support_y.shape[1]).to("cuda") / support_y.shape[1]
                
                for row_idx in range( len(query_x) ):
                    
                    max_entropy = self.calculate_entropy(equal_probs)
                
                    for task_id in range(data_manager_obj.current_task_id + 1):
                        
                        p = classification_decision[task_id][row_idx]
                        ...
                        if entropy < max_entropy:
                            ...
                            selected_label = p.argmax()
                            
                            max_entropy = entropy
                            
                    selected_col_idx = data_manager_obj.class_increase_per_task * selected_task_id + selected_label
                    
                    predictions[row_idx][selected_col_idx] = torch.tensor(1)
